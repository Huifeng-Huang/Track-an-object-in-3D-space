...
{  // compute distance ratios between all matched keypoints
    ...
    //double meanDistRatio = std::accumulate(distRatios.begin(), d
   ...
   std::cout <<" the time to colision calculated from camera is:" << TTC<< std::endl;
...
{
      //remove the points on the tailgate
 ...
 for ( auto point = lidarPointsPrev.begin(); point != lidarPointsPrev.end(); ++point)
 { if ( abs(point->y)>(laneWidth/2))
       lidarPointsPrev.erase(point);
 
 }
  
 for ( auto point = lidarPointsCurr.begin(); point != lidarPointsCurr.end(); ++point)
 { if ( abs(point->y)>(laneWidth/2))
       lidarPointsCurr.erase(point);
 
 }
       
   double curr_size = lidarPointsCurr.size();
   double prev_size = lidarPointsPrev.size();
   double curr_acc = 0;
  double prev_acc = 0;
  ...
  prev_acc += (*it).x;
   ...
   double curr_d = curr_acc/curr_size;
        double prev_d = prev_acc/prev_size;
       double dt = 1/frameRate;
       TTC = curr_d*dt/(prev_d-curr_d);
  std::cout <<" the time to colision calculated from lidar is:" << TTC<< std::endl;
...
void matchBoundingBoxes(std::vector<cv::DMatch> &matches, std::map<int, int> &bbBestMatches, DataFrame &prevFrame, DataFrame &currFrame)
{
  ...
  {cv::KeyPoint query = prevFrame.keypoints[(*it).queryIdx];
   ...
   for ( int j= 0; j <c; j++)
   {if ( currFrame.boundingBoxes[j].roi.contains(train.pt))
   {  trainID.push_back(j);
    train_found = true;}
   }
   
   if ( query_found == true && train_found== true)
   {
    for (auto i:queryID)
      for (auto j:trainID)
